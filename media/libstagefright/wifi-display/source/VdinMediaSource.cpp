...
VdinMediaSource::VdinMediaSource(uint32_t bufferWidth, uint32_t bufferHeight, bool canvas_mode) :
    ...
    ALOGV("VdinMediaSource: %dx%d", bufferWidth, bufferHeight);
...
status_t VdinMediaSource::read(MediaBuffer **buffer,
                                    const ReadOptions *options)
{
    ...
    *buffer = NULL;
    ...
    while (mStarted && mFramesReceived.empty()) {
        if (NO_ERROR !=
            ...
            ALOGW("Timed out waiting for incoming vdin media source frames: %lld us", mLastFrameTimestampUs);
            return -EAGAIN;
        }
    }
    if (!mStarted) {
    ...
    *buffer = tBuffer;
...
#if 0
    bufferTimeUs = mStartTimeUs + (mFrameCount * 1000000ll) / mFrameRate;

    int64_t current_time = systemTime(SYSTEM_TIME_MONOTONIC)/1000;

    if(abs(current_time- bufferTimeUs) > 33333 * 5){
        mStartTimeUs = current_time;
        mFrameCount = 0;
        bufferTimeUs = mStartTimeUs + (mFrameCount * 1000000ll) / mFrameRate;

        ALOGE("reset video pts to :%lld", bufferTimeUs);
    }
#endif
    ...
    //ALOGE("VdinMediaSource num:%lld latency:%lldms", cur_pts, (timeNow64 - cur_pts)/1000);
