...
TunnelRenderer::TunnelRenderer(
	...
	sp<IServiceManager> sm = defaultServiceManager();
    	...
    	mSystemWriteService = interface_cast<ISystemWriteService>(sm->getService(String16("system_write")));
    	if (mSystemWriteService == NULL) {
        ALOGE("Couldn't get connection to SystemWriteService\n");
        return ;
    	}
		int d = mSystemWriteService->getPropertyInt(String16("sys.wfddump"), 0) ;	
		d &= 0x02;
		if(d == 2)
			mDebugEnable = true;
		else
		...
		mSystemWriteService->setProperty(String16("sys.pkginfo"), String16("suc:0,fail:0,req:0,total:0, max:0,min:0,retry:0,band:0"));
}
		...
		if(mDebugEnable)
		{
			...
			if(curUs - mCurTime >= 1000000ll)
			{
				...
				sprintf(pkg_info, "suc:%d,fail:%d,req:%d, total:%lld, max:%lld,min:%lld,retry:%lld, band:%lld", 
	       	 	...
	       	 	mSystemWriteService->setProperty(String16("sys.pkginfo"), String16(pkg_info));
			}
		}
...
void TunnelRenderer::initPlayer() {
    ...
    mSystemWriteService->setProperty(String16("media.libplayer.wfd"), String16("1")); 
    mSystemWriteService->setProperty(String16("media.libplayer.fastswitch"), String16("2"));
    if(mIsHDCP) {
         Parcel data;
         data.writeInt32(1);
...
void TunnelRenderer::destroyPlayer() {
    ...
    mSystemWriteService->setProperty(String16("media.libplayer.wfd"), String16("0"));             
    mSystemWriteService->setProperty(String16("media.libplayer.fastswitch"), String16("0"));
    mPlayer->stop();
    mPlayer.clear();
