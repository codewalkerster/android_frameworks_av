...
sp<StreamFormatter> StreamFormatter::Create(
    ...
    } else if (!strcmp(codecMime, MEDIA_MIMETYPE_VIDEO_MPEG4)
            && (format == av_find_input_format("mp4")
                    || format == av_find_input_format("matroska"))) {
        return new MPEG42Formatter(codec);
    ...
    } else if (!strcmp(codecMime, MEDIA_MIMETYPE_VIDEO_VC1)
                && (format == av_find_input_format("asf")
                        || format == av_find_input_format("avi"))) {
        return new VC1Formatter(codec);
