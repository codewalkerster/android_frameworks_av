...
*****************************************************************************/
 ...
 uint32_t DTSDemuxBSInit(stDTSDemuxBitStrm * pstBit,       /* Bitstream context */
                     ...
                     uint32_t ui32BigEndian )          /* Flag to indicate whether the bitstream is bigendian or not */
 {
     ...
     pstBit->pui8IpBuff = pui8InputBuffer;
     pstBit->ui32IpBuffSize = ui32InputBufferLength;
     pstBit->ui32BitsLeft = 0;
     pstBit->ui32BuffOffset = 0;
     pstBit->ui32EOS = 0;
     ...
     {
         if(ui32BigEndian)
             pstBit->pFpGetWord = DTSDemuxReadWordCDFBigEndian;
         else
             pstBit->pFpGetWord = DTSDemuxReadWordCDFLittleEndian;
     }
     else
     {
         if(ui32BigEndian)
             pstBit->pFpGetWord = DTSDemuxReadWordBigEndian;
         else
             pstBit->pFpGetWord = DTSDemuxReadWordLittleEndian;
     }
 ...
 *****************************************************************************/
 ...
 uint32_t DTSDemuxBSGetBits (stDTSDemuxBitStrm * pstBit,   /* Bitstream context */
                         uint32_t ui32BitsReq )        /* BitsRequired */
 {
     ...
     if( ui32BitsReq <= pstBit->ui32BitsLeft)
     {
         /* Requested Bits are available in cache */
         pstBit->ui32BitsLeft -= ui32BitsReq;
         ui32Bits = (pstBit->ui32Cache >> pstBit->ui32BitsLeft) & aui32DemuxGetMask[ui32BitsReq];
     }
     else
     {
         if(NULL == pstBit->pFpGetWord)
         {
             return (0);
         }
         while(ui32BitsReq > 0 && !pstBit->ui32EOS)
         {
             /* Requested Bits are not available in cache so Load what is available */
             ui32Bits   = pstBit->ui32Cache & ( ( 1 << pstBit->ui32BitsLeft) -1);
 
             /* Update the request Bits */
             ui32BitsReq -= pstBit->ui32BitsLeft;
 
             ui32Bits <<= ui32BitsReq;
 
             /* Load 32 bits of data to cache */
             (*pstBit->pFpGetWord)(pstBit);
 ...
 * Exceptions        : None
 *****************************************************************************/
 void DTSDemuxReadWordCDFLittleEndian(void* pVoid)
 {
     uint32_t  ui32Word1;
     uint32_t  ui32Word2;
 
     stDTSDemuxBitStrm*  pstBit= (stDTSDemuxBitStrm *)pVoid;
 
     if(NULL == pstBit)
         return;
 
     if((pstBit->ui32BuffOffset + 3) >= pstBit->ui32IpBuffSize)
     {
         pstBit->ui32EOS = 1;
         return;
     }
     ...
     /* Read Data as two Words */
     ui32Word1   = (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+1];
     ui32Word1 <<= 8;
     ui32Word1   |= (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+0];
 
     ui32Word2   = (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+3];
     ui32Word2 <<= 8;
     ui32Word2   |= (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+2];
 
     pstBit->ui32Cache   = ui32Word1 & 0x3fff;
     pstBit->ui32Cache <<= 14;
     pstBit->ui32Cache  |= (ui32Word2 & 0x3fff);
 
     pstBit->ui32BuffOffset += 4;
     pstBit->ui32BitsLeft = 28;
 ...
 * Exceptions        : None
 *****************************************************************************/
 void DTSDemuxReadWordCDFBigEndian(void * pVoid)
 {
 ...
 * Exceptions        : None
 *****************************************************************************/
 void DTSDemuxReadWordLittleEndian(void * pVoid)
 {
     ...
     pstBit->ui32Cache   = (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+1];
     pstBit->ui32Cache <<= 8;
     pstBit->ui32Cache  |= (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+0];
     pstBit->ui32Cache <<= 8;
     pstBit->ui32Cache  |= (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+3];
     pstBit->ui32Cache <<= 8;
     pstBit->ui32Cache  |= (uint8_t)pstBit->pui8IpBuff[pstBit->ui32BuffOffset+2];
 
     pstBit->ui32BuffOffset += 4;
     pstBit->ui32BitsLeft = 32;
 ...
 * Exceptions        : None
 *****************************************************************************/
 void DTSDemuxReadWordBigEndian(void * pVoid)
 {
...
void DTSDemuxMatchDTSSyncType(uint32_t ui32TempSync, uint32_t ui32TempSync2, uint8_t* peDTSStreamFormat,
	                                        uint8_t* bCDFormat, uint8_t* bMotorola, uint8_t*g_24_wav_msb_flag)
{
	...
	*g_24_wav_msb_flag=0;
    ...
    {   
        *peDTSStreamFormat = SyncType_DTSDEMUX_SYNCWORD_CORE_16; /* 16-bit bit core stream*/
    }else if( ui32TempSync == DTSDEMUX_SYNCWORD_CORE_14 ){
        *bCDFormat = 1; 
        *peDTSStreamFormat = SyncType_DTSDEMUX_SYNCWORD_CORE_14; /* 14-bit bit core stream*/
    }else if( ui32TempSync == DTSDEMUX_SYNCWORD_CORE_16M ){
        *bMotorola = 1;
        *peDTSStreamFormat = SyncType_DTSDEMUX_SYNCWORD_CORE_16M; /* 16-bit bit core stream - Motorola format*/
    }else if( ui32TempSync == DTSDEMUX_SYNCWORD_CORE_14M ){
        *bCDFormat = 1; 
        *bMotorola = 1;
        *peDTSStreamFormat = SyncType_DTSDEMUX_SYNCWORD_CORE_14M;/* 32-bit sub stream Motorola*/
    }else if( ui32TempSync == DTSDEMUX_SYNCWORD_SUBSTREAM ){
        *peDTSStreamFormat = SyncType_Unknown;                   /* 32-bit sub stream*/
		...
		*g_24_wav_msb_flag = 1;
		*bMotorola = 1;
        *peDTSStreamFormat = SyncType_DTSDEMUX_SYNCWORD_CORE_24;/* 16-bit bit core stream - Motorola format*/
    }
}
...
void  DtshdExtractor::DtshdGetStreamParas()
{
	...
	uint8_t peDTSStreamFormat=SyncType_Unknown, bCDFormat=0,
	...
	{   //ALOGI("[%s %d] bytes_readed=%lld",__FUNCTION__,__LINE__,bytes_readed);
		for(i=0;i< bytes_readed-META_DATA_NEED_LEN;/*i++*/)
		{
			  ...
			  if(peDTSStreamFormat==SyncType_Unknown)
              {   
		      ...
		      {
                   DTSDemuxMatchDTSSyncType(ui32Sync_word, ui32Sync_word2, &peDTSStreamFormat,
			  ...
			  if(SyncTpyeDetectFlag==1)
              {
			       ...
			       uint16_t ui16FSIZE;
			       uint16_t ui32SFREQ;
			       stDTSDemuxBitStrm  pstBit={0};
			       int32_t bits_tmp,i32N;
			       int64_t FrameDurationUs;
			       uint8_t ui8IndexVecEnable=0;
                   if(peDTSStreamFormat==SyncType_DTSDEMUX_SYNCWORD_CORE_16 || peDTSStreamFormat==SyncType_DTSDEMUX_SYNCWORD_CORE_14 ||
		   	          peDTSStreamFormat==SyncType_DTSDEMUX_SYNCWORD_CORE_16M|| peDTSStreamFormat==SyncType_DTSDEMUX_SYNCWORD_CORE_14M)
                   {
		   	            if(ui32Sync_word==ui32SyncWordSave)
			   	            ui8IndexVecEnable=1;
                   }else if(peDTSStreamFormat==SyncType_DTSDEMUX_SYNCWORD_CORE_24){
			       ...
			       if(ui8IndexVecEnable)//need 11 bytes to get frame header info:
			       {
		   		       ...
		   		       DTSDemuxBSInit(&pstBit,&buf[i], BUF_LEN, bCDFormat,bMotorola);
		   		       DTSDemuxBSGetBits(&pstBit,32);//syncword
		   
		   		       bits_tmp = DTSDemuxBSGetBits(&pstBit,28);
...
//------------------------------------------
int DTSDemuxMatchDTSSync2(uint32_t ui32TempSync, uint32_t ui32TempSync2)
{
...
int Dtshd_probe( unsigned char *buf,int size)
{
   int i32Index=0;
   int result=0;
...
bool SniffDtshd(const sp<DataSource> &source, String8 *mimeType, float *confidence,
        sp<AMessage> *)
{
    ...
    if(Dtshd_probe(tmp,i32BytesToRead)>0)
	{
