...
uint32_t DTSDemuxRevBSInit(stDTSDemuxBitStrm * pstBit,		   /* Bitstream context */
...
uint32_t DTSDemuxBSChannelsSetInit(stDTSDemuxBitStrm * pstBit,	   /* Bitstream context */
...
/* Function to extract bits from the stream */
uint32_t DTSDemuxRevBSGetBits (stDTSDemuxBitStrm * pstBit,	  /* Bitstream context */
							  uint32_t i32Bits );			 /* BitsRequired */

/* Function to preview bits from the stream */
uint32_t DTSDemuxBSShowBits (stDTSDemuxBitStrm * pstBit,	/* Bitstream context */
							  uint32_t i32Bits );			 /* BitsRequired */

uint32_t DTSDemuxBSEOS ( stDTSDemuxBitStrm * pstBit);


/* Function to skip bits from the stream */
uint32_t DTSDemuxRevBSSkipBits ( stDTSDemuxBitStrm * pstBit,	/* Bitstream context */
							  uint32_t ui32SkipBits );			  /* Bits to skip */

/* Function to skip bits from the stream */
uint32_t DTSDemuxBSSkipBits ( stDTSDemuxBitStrm * pstBit,	 /* Bitstream context */
							  uint32_t ui32SkipBits );		  /* Bits to skip */

/* Function to flush remaining bits from cache used to word align the parsing*/
void	DTSDemuxBSFlush (stDTSDemuxBitStrm * pstBit);		 /* Bitstream context */	

/* Function to get the offset on stream */
uint32_t DTSDemuxBSGetOffset ( stDTSDemuxBitStrm * pstBit );		/* Bitstream context */

uint32_t DTSDemuxBSGetOffset1 ( stDTSDemuxBitStrm * pstBit );		 /* Bitstream context */

/* Function to read 28 bits in CD Format in Big Endian*/
void DTSDemuxReadWordCDFBigEndian (void *);

/* Function to read 28 bits in CD Format in Little Endian*/
void DTSDemuxReadWordCDFLittleEndian (void *);

/* Function to read 32 bits in Big Endian Format */
void DTSDemuxReadWordBigEndian (void *);

/* Function to read 32 bits in Little Endian Format */
void DTSDemuxReadWordLittleEndian (void *);

/* Function to ByteAlign the cache */
void DTSDemuxBSByteAlign( stDTSDemuxBitStrm *pstBit );

/* Function to skip specified number of bytes */
void DTSDemuxBSSkipBytes( stDTSDemuxBitStrm *pstBit, uint32_t ui32SkipBytes);

/* Function for bit stream intialisation pbr streams */
uint32_t DTSDemuxXLLPBRInit(stDTSDemuxBitStrm * pstBit,
						uint8_t*			pui8PBRBuff,
						uint32_t			ui32PBRBuffSize);

void DTSDemuxReadWordPBRBigEndian(void * pVoid);
void DTSDemuxReadWordPBRCDFBigEndian(void * pVoid);
void DTSDemuxReadWordPBRLittleEndian(void * pVoid);
void DTSDemuxReadWordPBRCDFLittleEndian(void * pVoid);
void DTSDemuxReadBytes(uint8_t* pui8Buff, uint64_t* pui64Data,uint32_t ui32Bytes);

#if defined (DTSDEC_XXCH) || defined (DTSDEC_X96)
void DTSDemuxBSReset(stDTSDemuxBitStrm *pstBitStrm,uint32_t ui32ByteOffset);
#endif
