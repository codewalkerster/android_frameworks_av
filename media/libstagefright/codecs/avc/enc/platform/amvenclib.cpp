        ...
        int mb_y, amvenc_pred_mode_obj_t *private_data)
{
    ...
    //amvenc_neighbor_t n;
    int i;
	amvenc_curr_pred_mode_t *cur = private_data->mb_node;
    amvenc_pred_mode_t      *common = private_data->mb_list;
...
static int fill_I_buffer(amvenc_drv_t* p, int slot)
{
    ...
    private_data->lambda_mode = motion_search->lambda_mode;
    private_data->lambda_motion  =motion_search->lambda_motion;
...
{
    amvenc_slot_t* cur_slot = (amvenc_slot_t*)&p->slot[slot];
	cur_slot->mbObj = (amvenc_pred_mode_obj_t *) calloc(1, sizeof(amvenc_pred_mode_obj_t));
	if(NULL == cur_slot->mbObj){
		ALOGD("failed to malloc space for curr MB\n");
	}else{
        cur_slot->mbObj->mb_list = &p->intra_mode;
        cur_slot->mbObj->mb_node = InitcurrMBStruct();
    }
    while(1){
        if(p->mCancel ==false)
            sem_wait(&(cur_slot->semstart));
        ...
        if(cur_slot->mode == Slot_mode_I_fill){
            cur_slot->mbObj->YCbCr[0]=p->src.pic[1].plane[0];
            cur_slot->mbObj->YCbCr[1]=p->src.pic[1].plane[1];
            cur_slot->mbObj->YCbCr[2]=p->src.pic[1].plane[2];
	...
	if(cur_slot->mbObj->mb_node){
		ClearcurrMBStruct(cur_slot->mbObj->mb_node);
        free( cur_slot->mbObj);
        cur_slot->mbObj = NULL;
	}
    return NULL;
...
static void* fillbuff_Thread_0(void *cookie)
{
    ...
    int slot = 0 ;
    prctl(PR_SET_NAME, (unsigned long)"AMVenc_SLOT0", 0, 0, 0);
...
static void* fillbuff_Thread_1(void *cookie)
{
    amvenc_drv_t *p = (amvenc_drv_t *)cookie;
    int slot = 1 ;
    prctl(PR_SET_NAME, (unsigned long)"AMVenc_SLOT1", 0, 0, 0);
    ALOGV("Thread slot %d is created \n" ,slot);	
    return _fillbuff_Thread(p,slot);
}
...
static AMVEnc_Status start_ime(amvenc_drv_t* p, unsigned char* outptr,int* datalen)
{
    ...
    offset[0] = offset[1] = 0 ;
    cmd = ENCODER_NON_IDR; 
    ioctl(p->fd, AMVENC_AVC_IOC_NEW_CMD, &cmd);
    while(p->mCancel == false){
        ...
        if(total_offset>offset[0]){
            ...
            ioctl(p->fd, AMVENC_AVC_IOC_FLUSH_CACHE ,addr_info);
            ioctl(p->fd, AMVENC_AVC_IOC_INPUT_UPDATE, &total_offset);
    ...
    ioctl(p->fd, AMVENC_AVC_IOC_GET_STAGE, &status);	
    while((status < ENCODER_IDR_DONE)&&(p->mCancel == false)){
        retry++;
        ioctl(p->fd, AMVENC_AVC_IOC_GET_STAGE, &status);	
    }
    ...
    if(status == ENCODER_IDR_DONE){
        ioctl(p->fd, AMVENC_AVC_IOC_GET_OUTPUT_SIZE, &size);	
...
static AMVEnc_Status start_intra(amvenc_drv_t* p, unsigned char* outptr,int* datalen)
{
    ...
    if((cmd == ENCODER_PICTURE)&&(status == ENCODER_PICTURE_DONE)){
        ioctl(p->fd, AMVENC_AVC_IOC_GET_OUTPUT_SIZE, &header_size);
...
static int GetAMVencRefBuf(int fd)
{
    int ret = -1;
    int addr_index = 0;
    if(fd>=0){
        ioctl(fd, AMVENC_AVC_IOC_GET_ADDR, &addr_index);
        if(addr_index == 1){
            ret= 0;
        }else{
            ret = 1;
...
amvenc_drv_t* InitAMVEncode(unsigned enc_width, unsigned enc_height, unsigned quant, unsigned mvrange, bool fs_en)
{
    int addr_index = 0;
    int ret = 0;
    ...
    if(!p){
        ALOGE("InitAMVEncode calloc fail");
        return NULL;
    }
    ...
    if(p->fd < 0){
        ALOGE("InitAMVEncode open encode device fail");
        free(p);
        return NULL;
    }
    memset(buff_info,0,sizeof(buff_info));
    ret = ioctl(p->fd, AMVENC_AVC_IOC_GET_BUFFINFO,&buff_info[0]);
    if((ret)||(buff_info[0]==0)){
        ALOGE("InitAMVEncode -- old amvenc driver.  no buffer information, use defalut value!");
    ...
    ioctl(p->fd, AMVENC_AVC_IOC_SET_QUANT,&p->quant);
    ioctl(p->fd, AMVENC_AVC_IOC_GET_ADDR, &addr_index);
	
    ioctl(p->fd, AMVENC_AVC_IOC_SET_ENCODER_WIDTH ,&p->enc_width);
    ioctl(p->fd, AMVENC_AVC_IOC_SET_ENCODER_HEIGHT ,&p->enc_height);
    if((p->enc_width != enc_width)||(p->enc_height != enc_height)){
        ALOGE("InitAMVEncode --set encode size fail. set as %dx%d, but max size is %dx%d.",enc_width,enc_height,p->enc_width,p->enc_height);
        munmap(p->mmap_buff.addr ,p->mmap_buff.size);
        close(p->fd);
        free(p);
        return NULL;
    }
    ioctl(p->fd, AMVENC_AVC_IOC_CONFIG_INIT,NULL);
    if(addr_index == 1){
        p->ref_id = 0;
    }else{
        p->ref_id = 1;
    ...
    p->motion_search.mvRange = mvrange;
    ...
    for(i = 0;i<2;i++){
        ...
        if(pthread_create(&(p->slot[i].mThread), &attr, thread_fun[i], p) !=0){
            ...
            ALOGE("InitAMVEncode create thread fail");
            CleanMotionSearchMoudle(&p->motion_search);
...
AMVEnc_Status AMVEncodeInitFrame(amvenc_drv_t* p, unsigned *yuv, bool IDRframe)
{
    AMVEnc_Status ret = AMVENC_FAIL;
    int i = 0;
    if((!p)||(!yuv))
    ...
    gettimeofday(&start_test, NULL);
#endif
    p->ref_id = GetAMVencRefBuf(p->fd);
	
    ALOGV("GetAMVencRefBuf id:%d",p->ref_id);
    ...
    if(p->IDRframe){
        ...
        //ALOGD("AMVEncodeInitFrame: NumIntraSearch:%d",NumIntraSearch);
    }
#ifdef DEBUG_TIME
    gettimeofday(&end_test, NULL);
    total_time = (end_test.tv_sec - start_test.tv_sec)*1000000 + end_test.tv_usec -start_test.tv_usec;
    p->total_encode_time +=total_time;
    ALOGD("AMVEncodeInitFrame: need time: %d us, ret:%d",total_time,ret);
#endif
    return ret;
}

AMVEnc_Status AMVEncodeSPS_PPS(amvenc_drv_t* p, unsigned char* outptr,int* datalen)
{
    AMVEnc_Status ret = AMVENC_FAIL;
    int mb_x, mb_y;
    uint8_t  *input_mb;
    unsigned cmd ,status;
    int mbx_max  = p->src.mb_width - 1;
    int mby_max  = p->src.mb_height - 1;
    unsigned update_bytes = 864 * (mbx_max+1)*10;
    int need_update = 0;
    unsigned offset[2]; //0 :  pre_offset   1: current_offset
    unsigned total_offset = 0, size = 0, retry = 0;
    unsigned addr_info[3];
    unsigned total_bytes = p->src.mb_width*p->src.mb_height*864;
    offset[0] = offset[1] = 0 ;
    cmd = ENCODER_SEQUENCE; 
    ioctl(p->fd, AMVENC_AVC_IOC_NEW_CMD, &cmd);
    for (mb_y = 0; mb_y <= mby_max; mb_y++){
        for (mb_x = 0; mb_x <= mbx_max; mb_x++){	
            ioctl(p->fd, AMVENC_AVC_IOC_GET_STAGE, &status);	
            if((cmd == ENCODER_SEQUENCE)&&(status == ENCODER_SEQUENCE_DONE)){
                if(p->gotSPS == false){
                    ioctl(p->fd, AMVENC_AVC_IOC_GET_OUTPUT_SIZE, &size);	
            ...
            if((cmd == ENCODER_PICTURE)&&(status == ENCODER_PICTURE_DONE)){
                cmd = ENCODER_IDR ;
                if((p->gotPPS == false)&&(p->gotSPS == true)){
                    ioctl(p->fd, AMVENC_AVC_IOC_GET_OUTPUT_SIZE, &size);	
                    if((size < p->output_buf.size)&&(size>0)){
                        p->gotPPS = true;
            ...
            input_mb = p->input_buf.addr + total_offset;
            total_offset += 864;
            fill_i_buffer_spec_default(p, input_mb, mb_x, mb_y);
            if((need_update)&&((total_offset%update_bytes) == 0)){
                if(total_bytes == total_offset)
                    total_offset = (total_offset+0xff)&0xffffff00;
                offset[1] = total_offset;
                addr_info[0] = ENCODER_BUFFER_INPUT;
    ...
    ioctl(p->fd, AMVENC_AVC_IOC_GET_STAGE, &status);	
    while(status < ENCODER_IDR_DONE){
        if((need_update)&&(offset[0]<total_offset)){
            if(total_bytes == total_offset)
                total_offset = (total_offset+0xff)&0xffffff00;
            need_update = 0;
            offset[1] = total_offset;
    ...
    ret = AMVENC_FAIL;
    ALOGV("AMVEncodeSPS_PPS status:%d", status);
    if(status == ENCODER_IDR_DONE){
        if((p->gotPPS == true)&&(p->gotSPS == true)){
            addr_info[0] = ENCODER_BUFFER_OUTPUT;
            addr_info[1] = 0 ;
            addr_info[2] = p->pps_len+p->sps_len ;
            ioctl(p->fd, AMVENC_AVC_IOC_FLUSH_DMA ,addr_info);
...
AMVEnc_Status AMVEncodeSlice(amvenc_drv_t* p, unsigned char* outptr,int* datalen)
{
    AMVEnc_Status ret = AMVENC_FAIL;
    if((!p)||(!outptr)||(!datalen))
        return ret;
...
void UnInitAMVEncode(amvenc_drv_t* p)
{
    void *dummy = NULL;
    int i = 0;
    if(!p)
