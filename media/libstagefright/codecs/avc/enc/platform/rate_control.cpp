...
static void updateRC_PostProc(AMVencRateControl *rateCtrl, MultiPass *pMP)
{
...
static void targetBitCalculation(amvenc_drv_t *p, AMVencRateControl *rateCtrl, MultiPass *pMP)
{
...
void calculateQuantizer(amvenc_drv_t *p, AMVencRateControl *rateCtrl, MultiPass *pMP)
{
...
static void updateRateControl(AMVencRateControl *rateCtrl, bool IDR)
{
    int  frame_bits;
...
AMVEnc_Status AMRCUpdateFrame(amvenc_drv_t *p, AMVencRateControl *rateCtrl, bool IDR)
{
...
void AMRCInitFrameQP(amvenc_drv_t *p, AMVencRateControl *rateCtrl)
{
    MultiPass *pMP = rateCtrl->pMP;
...
void AMRCUpdateBuffer(AMVencRateControl *rateCtrl, int frameInc)
{
    int tmp;
    MultiPass *pMP = rateCtrl->pMP;
...
void AMCleanupRateControlModule(AMVencRateControl *rateCtrl)
{
    if (rateCtrl->pMP)
        free(rateCtrl->pMP);
    rateCtrl->pMP = NULL;
...
AMVEnc_Status AMInitRateControlModule(AMVencRateControl *rateCtrl)
{
    if (rateCtrl->rcEnable == true)
    {
...
CLEANUP_RC:
    AMCleanupRateControlModule(rateCtrl);
