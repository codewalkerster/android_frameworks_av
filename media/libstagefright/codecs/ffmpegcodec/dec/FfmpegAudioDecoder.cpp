        ...
        if (mp3sync(header) == 0x7ff)
        {
                ...
                && (samplefreq != 3))
            {
                ...
                *framesize = ((mp3_samplenum_table[3-layer][verindex] >> 3) * bitrate) / samplefreq;
                ...
                i -= 4;
                //ALOGI ("mp3 frame size = %d, offset = %d, total = %d ... ", *framesize, i, size);
...
status_t FfmpegAudioDecoder::start(MetaData *) {
    ...
    ALOGI ("sample_fmt = %d, sample_rate = %d, channels = %d ... ", pCodecCtx->sample_fmt, pCodecCtx->sample_rate, pCodecCtx->channels);
...
status_t FfmpegAudioDecoder::decode (MediaBuffer **out) {
    ...
    switch (pCodecCtx->codec_id)
    {
        ...
        case CODEC_ID_MP3:
        {
            ...
            //ALOGI("mpeg audio buffersize = %d, offset = %d, framesize = %d ... ", bufferSize, offset, framesize);
    ...
    //ALOGI("decodesize = %d,  frameFinished = %d ... ", decodesize, frameFinished);
    ...
    if (frameFinished) {
        ...
        if (((pCodecCtx->sample_fmt != SAMPLE_FMT_S16) || (pCodecCtx->channels > 2)) && (mRsc == NULL)) {
            ALOGI("resample [sample_fmt, channels] from [%d, %d]  to [%d, %d] ... ", pCodecCtx->sample_fmt, pCodecCtx->channels, SAMPLE_FMT_S16, mNumChannels);
