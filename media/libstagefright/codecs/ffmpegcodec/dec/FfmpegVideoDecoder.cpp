...
#include "../../../../../../device/samsung/exynos4/include/sec_format.h"
        ...
        ALOGE("native_window_set_buffer_count, count = %d ", (minUndequeuedBufs+MAX_FRAMEBUFFER_NUMBER));
        ...
        for (uint32_t i = cancelStart; i < cancelEnd; i++) {
            ...
            if (tmp != OK) {
                ALOGE("cancelBuffer failed: %s (%d)", strerror(-tmp), -tmp);
            }
        }
    } 
    else
...
status_t FfmpegVideoDecoder::start (MetaData *) {
    ...
    ALOGI ("CODEC ID = 0x%x ... ", pCodecCtx->codec_id);
    ...
    ALOGI ("pixel format = %d, width = %d, height = %d ... ", pCodecCtx->pix_fmt, pCodecCtx->width, pCodecCtx->height);
    ALOGI ("time_base.den = %d, time_base.num = %d ... ", pCodecCtx->time_base.den, pCodecCtx->time_base.num);
    ...
    if ((mBufWidth * mBufHeight) > MAX_SUPPORT_RESOLUTION) {
        ALOGE ("can't support res : %d x %d ", mImgWidth, mImgHeight);
        av_free (pFrame);
...
status_t FfmpegVideoDecoder::stop() {
    ...
    ALOGI("<<< Statistics in FFmpegs Solution:");
    ALOGI("<<< Decode frame num = %lld, max_interval = %lldus, min_interval = %lldus, avr_interval = %lldus;\n",
            mDecFrameNum, mDecMaxInterval, mDecMinInterval, mDecSumInterval/mDecFrameNum);
    ALOGI("<<< Copy frame num = %lld, max_interval = %lldus, min_interval = %lldus, avr_interval = %lldus;\n",
            mCpyFrameNum, mCpyMaxInterval, mCpyMinInterval, mCpySumInterval/mCpyFrameNum);
    ALOGI("<<< Elapse time = %2f s, FPS = %.2f fps", timesec, (float)(mDecFrameNum/timesec));	
#endif	
...
status_t FfmpegVideoDecoder::decode () {
    ...
    while (mStarted) {
        ...
        err = OK;
        if (mSeekTimeUs >= 0) {
            ALOGI(" %s seek to %lld us ... ", __func__, mSeekTimeUs);
            /* flush and seek */
            if (mInputBuffer) {
                mInputBuffer->release();
                mInputBuffer = NULL;
...
#ifdef FPS
            ALOGI(" decode frame num = %lld, max_interval = %lld, min_interval = %lld, avr_interval = %lld\n",
                    mDecFrameNum, mDecMaxInterval, mDecMinInterval, mDecSumInterval/mDecFrameNum);
            ALOGI(" copy frame num = %lld, max_interval = %lld, min_interval = %lld, avr_interval = %lld\n",
                    mCpyFrameNum, mCpyMaxInterval, mCpyMinInterval, mCpySumInterval/mCpyFrameNum);
#endif

            ALOGI ("maybe end of stream ...... ");
            goto decode_return;
        }
        ...
        if (frameFinished) {
            ...
            if ((pCodecCtx->width != mImgWidth) || (pCodecCtx->height != mImgHeight)) {
                ...
                ALOGI("information changed : disp width = %d, disp height = %d ... ", mImgWidth, mImgHeight);
            ...
            if (mTargetTimeUs >= 0) {
                if (timeUs < mTargetTimeUs) {
                    skipFrame = true;
                    ALOGI("skipping frame at %lld us", timeUs);
                } else {
                    ALOGI("found target frame at %lld us", timeUs);
                    mTargetTimeUs = -1;
                }
            }
                ...
                if (pCodecCtx->pix_fmt == PIX_FMT_YUV420P) {
                    ...
                    if (mNativeWindow != NULL) {
                        ...
                        if (mapper.lock(bufferHandle, usage, bounds, vaddr) != 0) {
                            ALOGE("graphicbuffermapper.lock error");
                        ...
                        if (mapper.unlock(bufferHandle) != 0) {
                            ALOGE("graphicbuffermapper.unlock error");
                            mFinalStatus = ERROR_IO;
                            mBufferFilled.signal();
                            break;
                        }
...
status_t FfmpegVideoDecoder::read (MediaBuffer **out, const ReadOptions *options) {
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {
        ...
        ALOGI("FfmpegVideoDecoder::read : seek to %lldus ... ", mSeekTimeUs);

        //wait for response to the seek operation
        while (mSeekTimeUs >= 0) {
            if ((err = mBufferFilled.waitRelative(mLock, MAX_DECODE_WAIT_TIME)) != OK) {
                ALOGE("FfmpegVideoDecoder::read : waiting for seek operation time out ... ");
                return err;
            }
        }
    }
    ...
    while (mFilledBuffers.empty() && mStarted && (mFinalStatus == OK)) {
        if ((err = mBufferFilled.waitRelative(mLock, MAX_DECODE_WAIT_TIME)) != OK) {
            ALOGE("FfmpegVideoDecoder::read : waiting for decode time out ... ");
            return err;
        }
    }

    if (mFinalStatus != OK) {
        ALOGI("FfmpegVideoDecoder::read : mFinalStatus = %d ... ", mFinalStatus);
        err = mFinalStatus;
        mFinalStatus = OK;
        return err;
    }
    ...
    if (!mStarted) {
        ALOGI("FfmpegVideoDecoder::read : stop ... ");
...
void FfmpegVideoDecoder::signalBufferReturned(MediaBuffer *buffer) {
    ...
    if (buffer->graphicBuffer() == 0) {
        ...
        //ALOGI("graphic buffer release, index = %d", index);
        ...
        if (!rendered) {
            ALOGI("graphic buffer not queued, index = %d", index);
            mFreeBuffers.push_back(index);
            mBufferFreed.signal();
            return;
        }
        ...
        if (err != 0) {
            ALOGE("FfmpegVideoDecoder::signalBufferReturned: dequeueBuffer failed w/ error 0x%08x", err);
            return;
        }
        ...
        if (i >= mOutBuffers.size()) {
            ALOGE("FfmpegVideoDecoder::signalBufferReturned: dequeued unrecognized buffer ... ");
            return;
        }
        //ALOGI("graphic buffer dequeue index = %d", i);
        mFreeBuffers.push_back(i);
        mBufferFreed.signal();
    }
}
