      ...
      mEOSResult(OK)
{
    ...
    else if (strcmp (mime, MEDIA_MIMETYPE_VIDEO_AVC) == 0)
    {
        if (extradata[0] == 1)
        {
            ...
            ALOGI ("AVC stream, NAL length size = %d ... ", mNALLengthSize);
        }
        else
        {
...
status_t FfmpegExtractorTrack::read(MediaBuffer **out, const ReadOptions *options) {
    *out = NULL;
    ...
    if (!mPackets.empty()) {
        ...
        //    ALOGI("packet pts = %lld, target time = %lld ... ", timeUs, seekTimeUs);
...
bool FfmpegExtractorTrack::isMpeg2KeyFrame(const uint8_t *data, uint32_t size) {
    ...
    if ((sync == 0x100) && ((offset + 2) < size)) {
        ...
        ALOGI ("mpeg2 frame type %d ... ", pic_type);
        keyframe = (pic_type == 1);
    }
...
status_t FfmpegSource::read(MediaBuffer **out, const ReadOptions *options) {
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &seekMode)) {
        ALOGI ("track %d, seek to %lld ... ", mImpl->getTrackIndex(), seekTimeUs);
        if (seekTimeUs < 0) {
            seekTimeUs = 0;
        }
        seek = true;
...
readagain:
    ...
    if (mImpl->isMpeg2Stream()) {
        ...
        if ((buf[0] != 0) || (buf[1] != 0) || (buf[2] != 1)){
            ALOGE("mpeg2 packet error, release mediabuffer and read again ...... ");
            (*out)->release();
            goto readagain;
        }
        if (seek && !mImpl->isMpeg2KeyFrame(buf, (*out)->range_length())) {
            ALOGE("mpeg2 packet not key frame, release mediabuffer and read again ...... ");
            (*out)->release();
            goto readagain;
        }
    }
      ...
      mOffset(0)
{
    ...
    {
        ALOGE("FfmpegExtractor: av_probe_input_format error ... ");
        goto err_exit;
    }
    ...
    {
        ALOGE ("av_open_input_stream error .... ");
        goto err_exit;
    }
#else
    ...
    {
        ALOGE ("could not find codec parameters ... ");
        goto err_exit;
    }
    ...
    for (unsigned int i = 0; i < pAvformatCtx->nb_streams; i++ )
    {
        ...
        case AVMEDIA_TYPE_AUDIO:
            if (!haveAudio)
            {
                ...
                if (mimestr != NULL)
                {
                    ALOGI ("audio stream found : %s", mimestr);
                    ALOGI ("audio size = %d", cc->extradata_size);
                    if ((cc->extradata != NULL) && (cc->extradata_size > 0))
                    {
                        ...
                        for (int j=0; j<cc->extradata_size;)
                        {
                            ...
                            if ((j%8) == 0)
                            {
                                ALOGI ("audio extradata = %s", extrabuffer);
                    ...
                    ALOGI ("audio track channel count = %d, samplerate = %d ... ", cc->channels, cc->sample_rate);
                    ALOGI ("audio stream index = %d", audioIndex);
        ...
        case AVMEDIA_TYPE_VIDEO:
            if (!haveVideo)
            {
                ...
                if (mimestr != NULL)
                {
                    ALOGI ("video stream found : %s", mimestr);
                    ALOGI ("video size = %d", cc->extradata_size);
                    if ((cc->extradata != NULL) && (cc->extradata_size > 0))
                    {
                        char extrabuffer[48] = {0,};
                        for (int j=0; j<cc->extradata_size;)
                        {
                            ...
                            if ((j%8) == 0)
                            {
                                ALOGI ("video extradata = %s", extrabuffer);
                                memset (extrabuffer, 0, 48);
                            }
                        }
                        if ((cc->extradata_size % 8) != 0) {
                    ...
                    ALOGI ("video stream index = %d", videoIndex);
    ...
    if (strstr(fmt->name, "webm") != NULL) {
        mFileMeta->setCString(kKeyMIMEType, "video/x-matroska");
    } else if (strstr(fmt->name, "avi") != NULL) {
        mFileMeta->setCString(kKeyMIMEType, "video/avi");
    } else if (strstr(fmt->name, "rm") != NULL) {
        if (haveVideo) {
            mFileMeta->setCString(kKeyMIMEType, "video/x-pn-realvideo");
        } else {
            mFileMeta->setCString(kKeyMIMEType, "audio/x-pn-realaudio");
        }
    } else if (strstr(fmt->name, "asf") != NULL) {
        if (haveVideo) {
            mFileMeta->setCString(kKeyMIMEType, "video/x-ms-wmv");
        } else {
            mFileMeta->setCString(kKeyMIMEType, "audio/x-ms-wma");
        }	
    } else if (strstr(fmt->name, "mp4") != NULL) {
        if (haveVideo) {
            mFileMeta->setCString(kKeyMIMEType, "video/mp4");
        } else {
            mFileMeta->setCString(kKeyMIMEType, "audio/mp4");
        }
    } else if (strstr(fmt->name, "mpeg") != NULL) {
        mFileMeta->setCString(kKeyMIMEType, "video/mpeg");
    } else if (strstr(fmt->name, "flv") != NULL) {
        mFileMeta->setCString(kKeyMIMEType, "video/flv");
    } else if (strstr(fmt->name, "mov") != NULL) {
        mFileMeta->setCString(kKeyMIMEType, "video/mov");		
    } else {
...
err_exit:
    ...
    if (mTrackCount != 0)
    {
        ...
        ALOGI ("start time : %lld us ... ", mStartTime);
        mInit = true;
...
size_t FfmpegExtractor::countTracks() {
    if (!mInit) {
        ALOGE("FfmpegExtractor initialize error ... ");
        return 0;
    }
...
sp<MediaSource> FfmpegExtractor::getTrack(size_t index) {
    if (!mInit || index >= mTrackCount) {
        ALOGI("FfmpegExtractor::getTrack parameter error");
        return NULL;
    }
...
{
    ALOGI("FfmpegExtractor::getTrackMetaData ... ");
    if (!mInit || (index >= mTrackCount)) {
        return NULL;
    }
...
{
    ALOGI("FfmpegExtractor::getMetaData ... ");
    return mFileMeta;
}
...
status_t FfmpegExtractor::read()
{
    ...
    {
        mStartTime = tmp;
        ALOGI ("dts (%lld us) < mStartTime ... ", mStartTime);
        pkt.dts = 0;
    }
...
int64_t FfmpegExtractor::IOSeek( void *opaque, int64_t offset, int whence )
{
    ...
    if( i_absolute < 0 )
    {
        ALOGE("Trying to seek before the beginning");
        return -1;
    }

    if( i_size > 0 && i_absolute >= i_size )
    {
        ALOGE("Trying to seek too far : EOF?");
        return -1;
    }
...
bool SniffFfmpeg (const sp<DataSource> &source, String8 *mimeType, float *confidence, sp<AMessage> *) {
    ...
    *confidence = 0.0f;
    ...
    if( !( fmt = av_probe_input_format( &pd, 1 ) ) )
    {
        ALOGE("SniffFfmpeg:  av_probe_input_format error ... ");
        free (buffer);
        return false;
    }

    ALOGI(" >>>>>> SniffFfmpeg format : %s <<<<< ", fmt->name);
